...
{
    public class DarkEditStrategy : IPictureEditStrategy
    {
                ...
                if (OperatingSystem.IsWindowsVersionAtLeast(7)){
                    for (int x = 0; x < picture.Width; x++){
                        for (int y = 0; y < picture.Height; y++){
                            ...
                            var newPixel = Color.FromArgb(pixel.A,(int)Math.Max(0, pixel.R - 255 * amount), (int)Math.Max(0, pixel.G - 255 * amount), (int)Math.Max(0, pixel.B - 255 * amount));
